...
export async function catchupCommand(opts: CatchupOptions = {}): Promise<void> {
  ...
  try {
    await runCatchup(opts, async () => {
      if (lockLost !== undefined) return true;
      ...
      return !rows.some((l) => l.resource === lockKey && l.agent === 'catchup-command' && l.contextId === contextId);
    });
  } finally {
    renewal.stop();
...
async function runCatchup(opts: CatchupOptions, isLockLost: () => boolean | Promise<boolean>): Promise<void> {
  const lockKey = opts.topic ? `catchup:topic:${opts.topic}` : 'catchup';
