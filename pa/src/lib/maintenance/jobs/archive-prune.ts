  ...
  targets: [
    {
      ...
      action: 'delete',
      ownership: 'pa-owned',
      evidence: "PA's own rotated archive shards (~/.pa/archive/); PA is the sole writer (audit 2026-08-02). PRUNABLE_ARCHIVE_SUFFIXES is an explicit ALLOWLIST — rotated conversation-history shards are PERMANENT and excluded from both the age loop and the byte cap. Includes rotated turn-traces.jsonl shards from 2026-08-24 (deterministic per-run trace sidecar, AI-161) — derived debugging data, prunable at 90 days. Includes drained stall-records.jsonl shards from 2026-09-16 (staleness-check moves the live file here; diagnostic evidence, prunable at 90 days).",
