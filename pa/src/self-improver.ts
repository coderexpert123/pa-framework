...
export function buildReport(
  ...
  if (census) {
    const humanGated = census.families.filter((f) => f.classification === 'human-gated');
    const repeatUnchanged = census.families.filter((f) => f.classification === 'repeat-unchanged');
    ...
    if (humanGated.length > 0) {
      ...
      for (const f of humanGated) {
        ...
        lines.push(`- \`${f.family}\` (owner: ${f.owner ?? 'unknown'}) — ${ageDays}d old, last error: ${err}`);
      }
      lines.push('');
    }
    ...
    if (repeatUnchanged.length > 0) {
      ...
      for (const f of repeatUnchanged) {
        lines.push(`- \`${f.family}\` — sent ${f.sent}, ${f.distinctBodies} distinct bod${f.distinctBodies === 1 ? 'y' : 'ies'} — escalate / merge / mute`);
      }
      lines.push('');
    }
