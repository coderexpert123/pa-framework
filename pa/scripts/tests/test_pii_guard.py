    ...
    def test_help_flag_exits_zero_with_usage(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run(
                [sys.executable, guard_path, "--help"],
                stdin=subprocess.DEVNULL,
                capture_output=True,
                text=True,
                timeout=30,  # Should exit quickly; generous timeout avoids CI startup spikes.
                encoding="utf-8",
                errors="replace",
            )
    ...
    def test_bare_invocation_with_closed_stdin_exits_zero(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run(
                ...
                capture_output=True,
                text=True,
                timeout=30,  # Generous timeout avoids CI cold-start flakes.
                encoding="utf-8",
                errors="replace",
            )
        self.assertEqual(result.returncode, 0)
