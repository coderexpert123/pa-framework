...
class TestRestoreDrill(unittest.TestCase):
    ...
    @patch('run_restore_drill._get_or_create_folder')
    @patch('run_restore_drill._secret')
    def test_download_no_backups_found(self, mock_secret, mock_folder, mock_drive):
        ...
        self.assertRegex(str(ctx.exception), "No pa-secrets-.* backups found")
    ...
    def test_secret_raises_when_missing_and_required(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertRegex(str(ctx.exception), "TEST_SECRET not set")
            finally:
                if original:
