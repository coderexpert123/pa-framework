...
describe('watch-jobs', () => {
  ...
  describe('validateWatchInput (pure, no I/O)', () => {
    it('rejects a missing description', async () => {
      ...
      const input = { description: '', check: { type: 'file_exists', path: 'C:/x.txt' }, source: baseSource() };
    ...
    it('rejects a 201-char description and accepts 200', async () => {
      ...
      const r1 = validateWatchInput(
        { description: too_long, check: { type: 'file_exists', path: 'C:/x.txt' }, source: baseSource() } as WatchInput,
        T0,
      );
      ...
      const r2 = validateWatchInput(
        { description: exact, check: { type: 'file_exists', path: 'C:/x.txt' }, source: baseSource() } as WatchInput,
        T0,
      );
    ...
    it('rejects a missing source.chatId', async () => {
      ...
      const input = {
        description: 'ok',
        check: { type: 'file_exists', path: 'C:/x.txt' },
    ...
    it('rejects a missing pattern for file_contains', async () => {
      const { validateWatchInput } = await import('../src/lib/watch-jobs.js');
      const input = { description: 'ok', check: { type: 'file_contains', path: 'C:/x.txt' }, source: baseSource() };
      const r = validateWatchInput(input as WatchInput, T0);
      assert.equal(r.ok, false);
    ...
    it('rejects a pattern over 200 chars', async () => {
      ...
      const input = {
        description: 'ok',
        check: { type: 'file_contains', path: 'C:/x.txt', pattern: 'x'.repeat(201) },
        source: baseSource(),
      };
      const r = validateWatchInput(input as WatchInput, T0);
      assert.equal(r.ok, false);
    ...
    it('rejects an uncompilable regex pattern', async () => {
      ...
      const input = {
        description: 'ok',
        check: { type: 'file_contains', path: 'C:/x.txt', pattern: '(unclosed' },
        source: baseSource(),
      };
      const r = validateWatchInput(input as WatchInput, T0);
      assert.equal(r.ok, false);
    ...
    it('rejects a bad sinceIso for file_newer_than', async () => {
      ...
      const input = {
        description: 'ok',
        check: { type: 'file_newer_than', path: 'C:/x.txt', sinceIso: 'not-a-date' },
        source: baseSource(),
      };
      const r = validateWatchInput(input as WatchInput, T0);
      assert.equal(r.ok, false);
    ...
    it('defaults sinceIso to now for file_newer_than when omitted', async () => {
      const { validateWatchInput } = await import('../src/lib/watch-jobs.js');
      const input = { description: 'ok', check: { type: 'file_newer_than', path: 'C:/x.txt' }, source: baseSource() };
      const r = validateWatchInput(input as WatchInput, T0);
    ...
    it('applies defaults: intervalMs 60000, deadlineAt = now + 24h', async () => {
      const { validateWatchInput } = await import('../src/lib/watch-jobs.js');
      const input = { description: 'ok', check: { type: 'file_exists', path: 'C:/x.txt' }, source: baseSource() };
      const r = validateWatchInput(input as WatchInput, T0);
      assert.equal(r.ok, true);
    ...
    it('normalized check carries only its own type\'s fields', async () => {
      ...
      const input = {
        description: 'ok',
        check: { type: 'file_exists', path: 'C:/x.txt', pattern: 'should be dropped', pid: 999 },
        source: baseSource(),
      };
      const r = validateWatchInput(input as unknown as WatchInput, T0);
      assert.equal(r.ok, true);
      const check = (r as any).value.check;
      // normalizeWatchPath applies path.resolve(), which on win32 normalizes
      // forward slashes to backslashes — compare against the resolved form,
      // not the raw input string.
      assert.deepEqual(check, { type: 'file_exists', path: resolvePath('C:/x.txt') });
