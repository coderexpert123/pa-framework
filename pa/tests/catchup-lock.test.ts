...
before(async () => {
  ...
  await createMarkerSkill('lockloss-d', 'lockloss-topic-1');
  await createMarkerSkill('lockloss-e', 'lockloss-topic-1');
});
...
describe('catchupCommand lock-loss detection (D2/D3/D4, 2026-08-23)', () => {
  it('a purged catchup row aborts the tick at its next checkpoint and fires exactly one "Catchup aborted (lock lost)" notify', async () => {
    ...
    try {
      ...
      // fixed sleep). Once confirmed present, purge it: 5 overdue skills with
      // a 1s inter-dispatch stagger give ~4s of natural margin for the purge
      ...
      // Deterministic dispatch proof: since the checkpoint gained a live row
      // check (2026-08-28 push-gate incident), a purge landing before the
      // first dispatch correctly aborts with zero skills run. Purge only after
      // the FIRST skill's marker proves the dispatch loop is underway, so the
      // assertion below (≥1 ran, <5 ran) is guaranteed by construction
      // regardless of machine load or timer starvation.
      const markerDeadline = Date.now() + 30000;
      let firstRan = false;
      while (Date.now() < markerDeadline) {
        if (await readMarker('lockloss-a') !== null) { firstRan = true; break; }
        await new Promise((r) => setTimeout(r, 20));
      }
      assert.ok(firstRan, 'precondition: first skill must have run before the purge');
      ...
      // least one of the skills must have been skipped once the abort landed.
      const ran = (await Promise.all(
        ['lockloss-a', 'lockloss-b', 'lockloss-c', 'lockloss-d', 'lockloss-e'].map((n) => readMarker(n)),
      )).filter((v) => v !== null).length;
      assert.ok(ran >= 1, 'expected at least one already-dispatched skill to have run to completion');
      assert.ok(ran < 5, `expected at least one skill to be skipped once the lock was lost, but all ${ran} ran`);
      ...
      // Poll for notify attempt in app log to handle async notify execution and file flush
      let notifyAttempts: AppLogEntry[] = [];
      const logDeadline = Date.now() + 5000;
      while (Date.now() < logDeadline) {
        const pollEntries = await readAppLog();
        notifyAttempts = pollEntries.filter((e) => e.module === 'notify' && e.message === 'attempting' && e.subject === 'Catchup aborted (lock lost)');
        if (notifyAttempts.length >= 1) break;
