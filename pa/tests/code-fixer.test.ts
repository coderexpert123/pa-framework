...
describe('attemptCodeFix', () => {
  ...
  it('treats pa/data/profile* changes as ordinary dirty-worktree state (no special-casing, since AI-099 — profile.json no longer lives in the tracked tree)', async () => {
    await createTempSkill(dir, 'daily-mail-brief', '---\ncwd: "D:/fake-repo/projects/daily-mail-brief"\ncmd: "python scripts/run_brief.py"\n---\n\nBody.');
    const calls: ExecCall[] = [];
    let workerCalled = false;
    const exec = makeExec([
  ...
  it('stages and commits ONLY the worker-touched paths, never a bare git add -A', async () => {
    await createTempSkill(dir, 'daily-mail-brief', '---\ncwd: "D:/fake-repo/projects/daily-mail-brief"\ncmd: "python scripts/run_brief.py"\n---\n\nBody.');
    let postWorkerStatus = false;
    const calls: ExecCall[] = [];
    const exec = makeExec([
