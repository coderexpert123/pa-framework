...
describe('MAINTENANCE_JOBS registry', () => {
  ...
  it('declares exactly 27 jobs (18 pa + 9 bot) with the expected names', () => {
    assert.equal(MAINTENANCE_JOBS.length, 27);
  ...
  it('splits jobs correctly by host (18 pa, 9 bot)', () => {
    assert.equal(jobsForHost('pa').length, 18);
