...
describe('censusProposals (2026-08-23 alerts wave)', () => {
  ...
  it('suppressedBy \'fix-record\' deterministic-defect family ⇒ no proposal', () => {
    const census = makeCensus([makeFamily({ owner: 'daily-mail-brief', suppressedBy: 'fix-record' as const })]);
    assert.deepEqual(censusProposals(census, makeInput()), []);
  });

  it('suppressedBy \'green-signal\' ⇒ no proposal', () => {
    const census = makeCensus([makeFamily({ owner: 'daily-mail-brief', suppressedBy: 'green-signal' as const })]);
    assert.deepEqual(censusProposals(census, makeInput()), []);
  });

  it('regressedAfterFix family (not suppressed) ⇒ proposal still emitted', () => {
    const census = makeCensus([
      makeFamily({
