...
describe('pa claims --stats CLI layer', () => {
  ...
  it('pa claims --stats --json emits the CoordinationStats shape from synthetic log lines', async () => {
    ...
    // Seed the log DIRECTLY (deterministic) instead of via real claim():
    // claim()'s app-log write is async-buffered, and on slow CI disks
    // (ubuntu/windows runners) the immediate claimsCommand() read raced the
    // flush and saw zero claim lines — macOS' fast disk always won the race
    // (2026-08-29, PR #25-#28 CI-only failures). The claim() → log path is
    // already covered by the library tests above.
    await writeFile(
      join(dir, 'app.log.jsonl'),
      jsonLine({
        level: 'info', module: 'reservations', timestamp: new Date().toISOString(),
        message: 'claim granted', refId: 's-000000000099', id: 'r-cli-1',
        session: 'coord-audit', paths: ['pa/src/cli-stats.ts'], ttlMinutes: 45, forced: false,
      }),
      'utf8',
    );
