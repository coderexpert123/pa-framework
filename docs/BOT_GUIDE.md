...
| `claude-support`, `zclaude-support`, `codex-support`, `agy-support` (one per worker) | Debugging a specific worker — prompt engineering, API issues, worker-specific weirdness | `topic_defaults` map (each routes to that worker by default) |
...
4. **Add `topic_defaults` entries** to `~/.pa/config.yaml` so each topic defaults to the right worker:
   ...
   topic_defaults:
     ...
     "-100<group>_29": agy         # daily-briefings — agy for summaries
...
- `/help` — list commands
- `/agent [name]` — switch active agent harness (`agy`, `agyc`, `claude`, `codex`, `zclaude`) or view agent status
- `/model [name]` — view or override the active foundation model for the current agent (e.g. `/model gemini-3.8-flash-high`)
- `/effort [level]` — view or override agent reasoning effort (e.g. `/effort high`)
- `/default <agent | model name | setting value>` — set topic default agent or default tunable
- `/status` — display current topic status
...
### Pinned Topic Status Cards

Each forum topic receives a pinned status card generated by default on topic creation (and on `/branch` topics). The status card reflects:
- **Default Agent**: topic-configured default agent
- **Current Agent**: active agent handling dispatches
- **Model**: active underlying foundation model (e.g., `gemini-3.8-flash-high`, `opusplan`, etc.)
- **Reason**: status explanation (e.g., user override, failover, midnight reset)

### Per-Agent Model Memory & Failover Resolution

- **Sticky Memory**: Each topic remembers model preferences per agent independently. Switching agents (`/agent claude` -> `/agent agy`) automatically restores each agent's configured model.
- **Failover Resolution**: During automatic failover cascades, the failover runner evaluates `getExtraArgs` dynamically, passing the topic's preferred model args for each candidate agent.

Changes made via `/model`, `/agent`, `/default`, or `/reset` update the pinned status card in place immediately.

## Conversation archive + DLQ

### Archive (`~/.pa/conversation-history.jsonl`)

Every conversation turn is appended as a JSON line. This is the long-term log for skills like `memory-consolidation` to read and synthesize.
...
For agy and codex workers, the bot's `context.ts` constructs an equivalent prompt at runtime (no static file needed).
