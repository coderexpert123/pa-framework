...
- `/status` — display current topic status and keep-awake state
...
- **Keep-awake**: active machine sleep prevention state
...
Changes made via `/model`, `/agent`, `/default`, `/reset`, or `/keepawake` update the pinned status card in place immediately.
