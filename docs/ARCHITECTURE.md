...
| `cost_tier` | `'off_peak' \| 'anytime'` | `'anytime'` | `off_peak` skills run only in the z.ai off-peak window (configurable `cost_tier.peak_window_utc`); periodic off_peak skills defer during peak (once-daily log); time-pinned crons with `cost_tier` are ignored with a warning. |
...
The autonomous self-improvement loop (`pa/src/self-improver.ts`) gates skill prompt changes behind deterministic quality checks; a prompt fix that passes validation runs golden tasks:
...
The gate is SOFT in v1: pass applies the change and records the eval in the audit trail; fail parks it as `validation-failed-pending` for review via `pa improvements`. The validation floor still governs: a change failing validation stays pending regardless of eval.

Each task's `scorer.py` outputs JSON `{pass, detail}`, exit 0/1; results aggregate into `~/.pa/eval-results.jsonl`, and applied changes gain an `eval` field reviewable via `pa improvements --show <draft>`.
...
through it: the `watch_job` PA_META action and `pa watch add`. Bounds: 25 active,
10 checks/tick, 60 s-1 h intervals, 7-day max deadline, 256 KB tail reads. Terminal states
`reported`/`expired`/`check-failed`/`cancelled`; all but `cancelled` report to the
registering chat/thread, redacted and `_Ref:`-stamped, send before the status write.
...
`~/.pa/turn-traces.jsonl` — what the worker actually did, beyond the 300-char
preview the analyzer sees. Built by `pa/src/lib/turn-trace.ts`;
never blocks or fails a dispatch (`appendTurnTrace` never throws; fired with `void`).

Schema v1 (`TurnTraceV1`): `v`, `run_id` (uuid), `ts_start`/`ts_end`/
...
(`'ok'|'error'|'timeout'|'killed'|'failover'`), `parsed` (stream dialect recognized),
`tool_calls[]`/`commands[]`/`files[]`/`errors[]` (capped 200/50/50/5 entries, 200/300
chars per string), `retries` (always 0 in v1 — the future retry-loop seam),
`tokens`, `bytes_out`, `truncated`. Two dialects parse: agy/agyc `step_update` stream
events and claude-family `tool_use`/`tool_result` blocks; codex matches neither
(`parsed:false`) until it comes off cooldown.
...
Full-text search over six sources — answers "did we discuss this" without grepping raw
JSONL:
...
Wave-1 substrate of the topic-task handover (2026-09-02):

- **`pa/src/lib/topic-tasks.ts`** — durable per-topic task queue at `~/.pa/topic-tasks/<chatId>_<threadId>.json`. `appendTask` dedups by content hash (double-queueing cannot double-execute); pop persists before injecting (Wave-2's `claimNextTask` moves records into the running store under the same lock). Read-modify-writes run under proper-lockfile + in-process mutex via `writeJsonAtomic`; `validateTaskPrompt` enforces non-empty, single line, ≤500 chars, no leading `/`.
...
(route/telemetry/input/complete/transcribe) write the same ledger directly and never
create schema. Voice recordings live under files/<task_id>/, transcribed by
the topic worker (Groq-first) — the API server carries no transcription secrets.
`route_task.py` also writes one decisions.sqlite row per routing decision (the
explainability feed). Operator input arrives as one of six typed input-request widgets;
answers are stored server-side as a file and delivered to the worker as a path, so secrets
...
makes outbound connections only — its poller long-polls the worker, executes claimed
requests against the localhost API, and delivers responses back. Free-tier caps set the
shape: long-poll waits ≤25 s, a parked request dies at 55 s, and bodies over 768 KiB
stream through R2. The relay is generic: any localhost HTTP service can front through
it. Component detail, auth model, and gates: `projects/voice-inbox/relay/README.md`.
...
- **Daily digest skill** — deterministic 24h activity rollup (runs, tokens, est. cost, failover, alerts, DLQ, self-improver actions, SLO) with explicit unavailable lines; Python → stdout → `telegram_output` relay, date-scoped dedup. Script: `pa/scripts/daily_digest.py`; template: `examples/skills/daily-digest/skill.md`. Schedule/destination are per-deployment frontmatter.
