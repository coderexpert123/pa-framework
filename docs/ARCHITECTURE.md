...
| `cost_tier` | `'off_peak' \| 'anytime'` | `'anytime'` | Cost tier for scheduled skills. `off_peak` skills run only during the z.ai off-peak window (19:30-11:30 IST; currently fixed, not configurable); periodic off_peak skills are deferred during peak hours (11:30-19:30 IST) with once-daily logging. Time-pinned crons with `cost_tier` are ignored with a config warning. |
