...
- **`staleness-check` is THE dead-man's switch** for scheduled skills — `skill-cadence-audit` (retired) was a strictly-later duplicate that suppressed nothing. Skips parked skills, widens for `cost_tier: off_peak` (z.ai peak window configurable via `cost_tier.peak_window_utc` in config.yaml), dedups on the stale SKILL SET (re-alert on change).
