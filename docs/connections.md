...
- **`staleness-check` is THE dead-man's switch** for scheduled skills — `skill-cadence-audit` (retired) was a strictly-later duplicate that suppressed nothing. Skips parked skills, widens for `cost_tier: off_peak` (fixed z.ai peak window not configurable), dedups on the stale SKILL SET (re-alert on change).
