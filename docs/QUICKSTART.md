...
### Activate the bot's standing rules (required for claude/zclaude workers)

The bot's standing rules (capabilities, routing behavior, response format) live in `projects/telegram-bot/bot-instructions.md`. **Without this step, claude/zclaude worker prompts omit the rules block entirely** — the bot runs with no context about how to behave.
