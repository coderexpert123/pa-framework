...
Placement is the act of choosing that home between the only two trees this framework has. The public mirror is the published framework that any reader may copy. The private repo is the working repository of one operator, the person the assistant serves, and it holds that deployment's data and wiring. The rule is that placement is decided once, when the feature is built, and recorded then. Nothing mechanical enforces that rule; the recording procedure in the last section and the audit described there are the process that carries it.
