...
The default matches the Zhipu (z.ai) peak billing window (Mon-Fri 06:00-10:00 UTC, 2x credits). `off_peak` skills defer during peak and run off-peak; customize to align with another provider's schedule.

**Non-wrapping (v1):** `start_hour` < `end_hour` required; wrap-around windows (e.g. 22:00-02:00) warn and fall back to the default.
...
Key resolution: try `record.model` (trimmed), else `record.worker`. Config merges over built-ins key-by-key. Thinking tokens bill at the output rate; `cache_read` defaults to 0. An unpriced key → `estCostUsd: null`.
...
**`PA_KB_SOURCES_PATH` is also read by `pa recall`** (2026-08-24): a *file* path; recall indexes its *directory*. Unset skips the KB source — the same "unset means off" convention `kb-notes.ts` documents. As of 2026-08-24 the new env vars are `PA_BOT_SELF_RESTART`, `PA_OPERATOR_USER_ID`, `PA_RICH_MESSAGES` (all above) — no other new knob exists.
...
**Multi-instance and the OS scheduler (2026-07-23):** file state was always per-`PA_HOME`, but `pa schedules sync`'s scheduler entry used a fixed name — a second install silently overwrote the first's task. Fixed: with the default `~/.pa` the names stay `PA-Catchup`/`PA-Catchup-Reminders` (single-install unchanged); any OTHER resolved path gets its own hash-suffixed name, so two installs can never collide. See `docs/QUICKSTART.md` §11 and `pa schedules list` for the name in effect.

## Resource tuning env vars

| Variable | Default | Purpose |
|----------|---------|---------|
| `PA_MAX_CONCURRENT_WORKERS` | `3` | Machine-wide cap on concurrent LLM workers (bot + skills share the pool via slot locks); excess queues.lot frees. Set `0` or negative to disable limiting. Evaluator calls are exempt (they run while a slot-holding worker awaits their verdict). Shell/`cmd:` skills are unaffected. |
| `UV_THREADPOOL_SIZE` | Node default `4` | Recommended `16` for bot + catchup: fs and DNS share the libuv pool; heavy disk I/O can starve DNS.g down with it. Set it in the process launcher (Task Scheduler wrapper, systemd unit, shell profile) — it must exist before Node starts. |
...
| `PA_ALERT_CENSUS_NOTIFY_PER_DAY` | `50` | `alert-census` job threshold (daily, pa host). Always writes `~/.pa/alert-census.json`; pages above the cap.ensus (7d)" summary to pa-alerts only when `totalSent / windowDays` is at or above this value. |
...
| `PA_BUILD_LOCK` | unset (lock ON) | `0` skips the automatic `@build` reservation of `npm run build`/`npm test` — for scoped runs inside an orchestrated wave.builders would otherwise serialize behind one another. Never set it for a full-suite or pre-push gate. |
...
| `PA_LOCK_RENEW_MAX_MS` | `21600000` (6 h) | Absolute cap on `startLockRenewal()`; past it the lock goes stale normally.nism. |
| `PA_PUBLIC_SYNC_LOCK_WAIT_MS` | `300000` (5 min) | `public-sync` wait for the git-public lock before exit 5 (busy); test-only.rride in practice — leave unset in production. |
...
| `PA_BOT_SELF_RESTART` | unset (enabled) | `0` disables `bot-self-restart` — the graceful restart the bot performs when its dist stamp is newer than the process.ild-stamp` is newer than the running process start time and the bot is idle (no dispatch, no pending action, no topic lock, `@build` free). Never in-process — Task Scheduler relaunches on the newer `dist/`. See `docs/maintenance-jobs.md`. |
