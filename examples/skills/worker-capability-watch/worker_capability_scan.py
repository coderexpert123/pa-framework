...
have fails EVERY dispatch to that worker and presents as an outage, not a
...
def check_shim_integrity(shim_dir: Path, baseline_path: Path,
    ...
    Args:
        shim_dir: Path to shim directory (no default; check skipped unless provided)
    ...
    if shim_dir is None or not shim_dir.exists():
        return None
...
def render_report(findings: list[dict]) -> str:
    ...
    if breaking:
        ...
        lines.append("")
        home = pa_home()
        lines.append(f"Fix `{home}/config.yaml` by hand. Nothing was "
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--shim-dir", help="Path to shim directory.")
    ...
    shim_dir = Path(args.shim_dir) if args.shim_dir else None
