...
// .mjs entry points of the packages that have them — separate entry points in
// separate packages with no shared module. Only this
...
function writeBuildStamp() {
  try {
    ...
    writeFileSync(join(dist, '.build-stamp'),
      JSON.stringify({ builtAt: new Date().toISOString(), sha, pkg: 'voice-inbox' }) + '\n', 'utf8');
  } catch (e) {
...
async function main() {
  ...
  process.exit(await withBuildLockOrRun(repoRoot, 'voice-inbox', run));
}
