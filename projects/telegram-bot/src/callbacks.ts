  ...
  try {
    ...
    switch (parsed.prefix) {
      case 'reauth': {
        ...
        if (messageId) await editMessageReplyMarkup(deps.token, chatId, messageId, undefined);
        await sendMessage(
          deps.token,
          chatId,
      ...
      case 'rm': {
        if (parsed.action === 'done') {
          // A due reminder is deleted before it is sent (spec §3.4 / correction 5) —
          // nothing to write. Pure acknowledgement.
