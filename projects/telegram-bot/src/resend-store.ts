...
export interface ResendRecord {
  ...
  messageId: number;
  userText: string;
  /** Propagated from PendingDispatch.userTextSettled by the reaper's putResend so
   * the rs: guard can tell a lost transcript from a recovered one. Optional —
   * legacy records load with it absent (2026-08-27 honest-resend spec). */
