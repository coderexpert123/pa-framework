...
- **Feedback Rules (2026-08-28, AI-165)**: `context.ts` injects `## Standing rules` (active rules matching global/thread, 12-rule / 1500-char cap); `rules-critic.ts` runs per-reply deterministic check logging to `rules-violations.jsonl`. CLI: `pa rules`.
