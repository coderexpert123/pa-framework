...
- **Premature-async-reply guard (AI-202, 2026-09-04)**: `isPrematureAsyncReply`
  ...
  option-B outcome) at EVERY parseMetadata delivery site — `main.ts` (blanked into
  the empty-response error), `orphan-reaper.ts` ×4 (fall through to the next source),
  `task-executor.ts` (rides the retry ladder, never the completion FYI). Always
...
- **Raw-send guard (2026-09-04)**: cross-topic delivery contract (`pa notify
  ...
  attaches `rawTelegramSends` to CommandResult; `main.ts` fires ONE best-effort
  pa-support alert — alert-only, never blocks the reply.
