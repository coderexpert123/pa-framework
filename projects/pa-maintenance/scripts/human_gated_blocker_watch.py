...
Concretely, `zclaude` (priority 1) has been `account-exhausted` (a Zhipu billing
fault documented in rate-limits.ts as one that "never self-heals") for well over
a week, and the only signal is a dedup-suppressed one-shot that never escalates.
...
Usage:  python "C:/pa-checkout/projects/pa-maintenance/scripts/human_gated_blocker_watch.py"
"""
...
def page_alerts(report: str, reply_markup: dict | None = None) -> None:
    ...
    `pa ref` can resolve it. reply_markup (optional InlineKeyboardMarkup dict)
    rides the LAST chunk (WP-D2 B.1, 2026-09-02)."""
...
def main(argv=None) -> int:
    ...
    # WP-D2 B.1 (2026-09-02): a reauth page carries the one-tap /reauth button.
    # The callback_data must parse against the single grammar (reauth: prefix,
    # chat-gated — the bot's callbacks.ts handles it like the typed /reauth).
    reply_markup = None
    if google_auth_alerts:
    ...
    if (alerts or postmortem_alerts or google_auth_alerts) and not args.no_send:
        try:
            page_alerts(report, reply_markup=reply_markup)
