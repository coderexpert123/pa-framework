...
for attempt in range(2):
    ...
    if attempt == 0:
        ...
        fail_llm(f'no markers after 2 attempts: {candidate[:300]}')

if response is None:
    fail_llm('no response')
...
if not briefing_output or not briefing_output.startswith('[pa assert]'):
    fail_llm(f'malformed content: {briefing_output[:300] or "<empty>"}')
